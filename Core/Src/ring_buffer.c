...
uint8_t validate_key(ring_buffer_t *buffer) {
	char correct_key[] = "1002653824";
    uint8_t size = ring_buffer_size(buffer);

    if (size != (sizeof(correct_key)-1)) {
        return 0; // Tamaño no coincide
    }

    for (uint8_t i = 0; i < size; i++) {
        uint8_t key;
        if(ring_buffer_read(buffer, &key) == 0){
        	return 0;//buffer vacio
        }

        if (key != correct_key[i]) {
